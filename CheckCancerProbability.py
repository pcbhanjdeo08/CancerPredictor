...
# Initialize model and accuracy as None
_model = None
_accuracy = None
...
def _train_model():
    ...
    _accuracy = accuracy_score(y_test, predictions)

def CheckCancerProbability(input_array):
    """Calculate cancer probability from input array"""
    _train_model()  # Ensure model is trained
    ...
    input_data = np.array([[float(input_array[0]),
                            ...
                            float(input_array[2]),
                            float(input_array[3])]])
    probability = _model.predict_proba(input_data)[0][1]
    return {
        "simulated_cancer_probability": round(float(probability), 4),
        "model_accuracy": round(float(_accuracy), 2),
